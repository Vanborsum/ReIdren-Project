...
def process_video(cloud_event):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        for post_string in social_media_posts.splitlines():
            post_string = post_string.strip()
            if len(post_string) == 0:
                continue
            payload = {
                "post_content": post_string
            }
            response = requests.post("https://hooks.zapier.com/hooks/catch/6711331/2lpe7o3/", json=payload)

            # Check if the request was successful
            if response.status_code == 200:
                print("Post data sent to Zapier successfully!")
            else:
...
def generate_social_media_posts(transcript):
    ...
    messages = [
        ...
        {"role": "user", "content": (
            ...
            "These should not try to advertise the video, but rather advertise the content of the video, each post should begin on a new line "
            "such that each line can be cut away and formed into a social media post:\n\n"
            f"{transcript}"
        )}
    ]
